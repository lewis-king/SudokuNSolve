...
import {calculateGridPositions} from '../services/SimpleGridPositionCalculator';
...
export default class CameraTextRecognition extends Component {
  state = {
    loading: false,
    image: null,
    error: null,
  ...
  processImage = async (uri, imageProperties) => {
    ...
    //});
    const grid = this.determineGridFromImage(visionResp);
    const {navigate} = this.props.navigation;
    navigate('SudokuGrid', {type: 'solve', grid});
  };

  determineGridFromImage = visionResp => {
    return calculateGridPositions(visionResp);
  };
